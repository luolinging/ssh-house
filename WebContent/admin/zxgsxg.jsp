...
new CommDAO().update(request,response,"zxgs",ext,true,true); 
HashMap mmm = new CommDAO().getmap(id,"zxgs"); 
%>
<body>
  <form  action="zxgsxg.jsp?f=f&id=<%=mmm.get("id")%>"     name="f1" method="post"  onsubmit="return checkform()" >
...
var gsnameobj = document.getElementById("gsname");  
if(gsnameobj.value!=""){ shhousejax = new AJAX(); 
ajax.post("/lpsqglpt/factory/checkno.jsp?table=zxgs&col=gsname&value="+gsnameobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>"); 
