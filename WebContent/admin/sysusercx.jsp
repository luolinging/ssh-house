...
&nbsp;&nbsp;&nbsp;<input type=submit class='' value='查询信息' /> &nbsp;&nbsp;<input type=button   class='' value='添加信息' onclick="window.location.replace('adduser.jsp')" /> &nbsp;&nbsp;</td>
                </tr>
            </table></td>
      </tr>
    </table>
...
<td align="center" class="main_title_td_center2">身份证号码</td>
...
<tr 
            ...
            <td align="center" valign="middle" class="main_matter_td"><a href="updateuser.jsp?id=<%=map.get("id")%>">修改</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="return confirm('确定要删除这条记录吗?')" href="sysusercx.jsp?scid=<%=map.get("id")%>">删除</a></td>
</tr>
<%}%>
 ...
 <script language=javascript src='/lcdriverteach/js/ajax.js'></script>
<% 
...
<script language=javascript src='/lcdriverteach/js/My9shhousedatePicker.js'></script>
<script language=javascript src='/lcdriverteach/js/popup.js'></script>
<script language=javascript> 
function update(no){ 
pop('updateuser.jsp?id='+no,'信息修改',550,280) 
}
</script> 
<script language=javascript> 
function add(){ 
pop('adduser.jsp','信息添加',550,280) 
}
</script> 
<%@page import="util.Info"%> 
