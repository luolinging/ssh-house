...
<body>
  <form  action="dqidatadiccx.jsp"  name="f1" method="post"  >
...
<input type=button   class=''  value='添加信息' onclick="add();" /> &nbsp;&nbsp;</td>
      ...
      <table width="100%" border="0" 
          ...
          <tr 
              onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#f0f7ff'" 
              onmouseout="this.style.backgroundColor=currentcolor">
            <td align="center" class="main_title_td_center2">地区名称</td>
...
String sql = "select * from idatadic where 1=1 " ;
 ...
 sql+= " and icontent <'"+Info.getDay(endicontent,1)+"' " ;
  ...
  sql +="  and (1!=1 " ; 
  sql +="  or  itype like'%楼房地区%' " ;
  sql +="  ) ";
  sql +=" order by id desc ";
String url = "dqidatadiccx.jsp?1=1&icontent="+picontent+""; 
...
&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
<a onclick="return confirm('确定要删除这条记录吗?')"  href="dqidatadiccx.jsp?scid=<%=map.get("id")%>">删除</a>
</td>
</tr>
<%}%>
...
<script language=javascript> 
function update(no){ 
pop('dqidatadicxg.jsp?id='+no,'信息修改',550,'100%') 
}
</script> 
<script language=javascript> 
function add(){ 
pop('dqidatadictj.jsp','信息添加',550,'100%') 
}
</script> 
<%@page import="util.Info"%> 
