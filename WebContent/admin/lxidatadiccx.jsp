...
<body>
  <form  action="lxidatadiccx.jsp"  name="f1" method="post"  >
...
String sql = "select * from idatadic where 1=1 " ;
 ...
 sql+= " and icontent <'"+Info.getDay(endicontent,1)+"' " ;
  ...
  sql +="  and (1!=1 " ; 
  sql +="  or  itype like'%楼房类型%' " ;
  sql +="  ) ";
  sql +=" order by id desc ";
String url = "lxidatadiccx.jsp?1=1&icontent="+picontent+""; 
...
&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
<a onclick="return confirm('确定要删除这条记录吗?')"  href="lxidatadiccx.jsp?scid=<%=map.get("id")%>">删除</a>
</td>
</tr>
<%}%>
...
<script language=javascript> 
function update(no){ 
pop('lxidatadicxg.jsp?id='+no,'信息修改',550,'100%') 
}
</script> 
<script language=javascript> 
function add(){ 
pop('lxidatadictj.jsp','信息添加',550,'100%') 
}
</script> 
<%@page import="util.Info"%> 
